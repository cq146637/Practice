...
class AuditLog(Base):
    ...
    '''
'''
class AuditLog(modules.Model):
    session = modules.ForeignKey(SessionTrack)
    user = modules.ForeignKey('UserProfile')
    host = modules.ForeignKey('BindHosts')
    action_choices = (
        (0,'CMD'),
        (1,'Login'),
        (2,'Logout'),
    ...
    action_type = modules.IntegerField(choices=action_choices,default=0)
    cmd = modules.TextField()
    memo = modules.CharField(max_length=128,blank=True,null=True)
    date = modules.DateTimeField()
